...
CPluginMngr::CPlugin* CPluginMngr::loadPlugin(const char* path, const char* name, char* error, int debug , const char * shortName )
{	
	CPlugin** a = &head;
	...
	*a = new CPlugin(pCounter++, path, name, error, debug , shortName );
...
void CPluginMngr::CPlugin::Finalize()
{
	...
	if (CheckModules(&amx, buffer))
	{
		if (amx_Register(&amx, core_Natives, -1) != AMX_ERR_NONE)
		{
			...
			{
				status = ps_bad_load;
				sprintf(buffer, "Plugin uses an unknown function (name \"%s\") - check your modules ", no_function);
