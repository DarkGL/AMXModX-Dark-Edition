...
plugin_info_t Plugin_info = 
{
	...
	"AMX Mod X Dark Edition",				// name
	SVN_VERSION_STRING,			// version
	__DATE__,					// date
	"AMX Mod X Dev Team @Edited Rafal DarkGL Wiecek",		// author
	"http://www.amxmodx.org and www.darkgl.amxx.pl",	// url
	"AMXX",						// logtag
...
cvar_t init_amxmodx_dark	=	{ "amxmodx_dark" "" , FCVAR_SERVER | FCVAR_SPONLY };
...
// Initialize AMX stuff	and	load it's plugins
...
int	C_Spawn(edict_t *pent)
{
	...
	get_localinfo_r( "amxx_pluginsdir" , "addons/amxmodx/plugins" , pluginsDir, sizeof( pluginsDir )  );

	snprintf( debugPath , sizeof( debugPath ) , "%s/debug" , pluginsDir );

	g_plugins.loadPluginsFromDir( pluginsDir , false );
	g_plugins.loadPluginsFromDir( debugPath , true );
...
C_DLLEXPORT	int	Meta_Attach(PLUG_LOADTIME now, META_FUNCTIONS *pFunctionTable, meta_globals_t *pMGlobals, gamedll_funcs_t *pGamedllFuncs)
{
	...
	CVAR_REGISTER(&init_amxmodx_dark);
	...
	// ###### Print short GPL
	print_srvconsole("\n   AMX Mod X version %s Copyright (c) 2004-2006 AMX Mod X Development Team Edited by DarkGL \n"
