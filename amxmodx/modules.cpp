...
extern const char*	get_localinfo_r(const char *name, const char *def, char buffer[], size_t maxlength);
...
int load_amxscript(AMX *amx, void **program, const char *filename, char error[64], int debug , const char * shortName )
{
	*error = 0;
	size_t bufSize;
	*program = (void *)g_plugins.ReadIntoOrFromCache(filename, bufSize);
	...
	if ((int)CVAR_GET_FLOAT("amx_debug") >= 2 || debug)
	{
		...
		if( ((hdr->flags & AMX_FLAG_DEBUG) != 0) ){
			AMXXLOG_Log( "[AMXX] Plugin \"%s.amxx\" compiled with debug option you can optimize it through compile it with -d0 or -d1 flags" , shortName ); 
		}
	}
...
# endif
	...
	if (g_plugins.m_Finalized)
	{
		...
		if (CheckModules(amx, error))
		{
			...
			{
				sprintf(error, "Plugin uses an unknown function (name \"%s\") - check your modules.", no_function);
				return (amx->error = AMX_ERR_NOTFOUND);
			}
		} else {
			return (amx->error = AMX_ERR_NOTFOUND);
...
int CheckModules(AMX *amx, char error[128])
{
	...
	for (int i = 0; i < numLibraries; i++)
	{
		...
		if (!found)
		{
			...
			sprintf(error, "%s \"%s\" required for plugin. Check modules.", type, buffer);
			return 0;
		}
	}
				...
				if ( (err=RunLibCommand(&dec)) != LibErr_None )
				{
					if (!pHandler->HandleModule(dec.param1, (err == LibErr_NoClass)))
					{
						...
						if (err == LibErr_NoClass)
							type = "Module/Library Class";
						sprintf(error, "%s \"%s\" required for plugin.  Check modules.", type, dec.param1);
						return 0;
					}
				}
			}
...
int loadModules( PLUG_LOADTIME now )
{
	int loaded = 0;
	...
	#if defined WIN32
	build_pathname_r( dirModules , sizeof( dirModules ) , "%s\\*" , get_localinfo("amxx_modulesdir", "addons/amxmodx/modules" ) );

	_finddata_t fd;
	intptr_t handle = _findfirst( dirModules , &fd);
	...
	while (!_findnext(handle, &fd)){
		...
		if ( LoadModule( fd.name , now, false ))
			loaded++;
	}
	...
	while ( ( ep = readdir( dp )) != NULL ){
		...
		if (LoadModule( ep -> d_name , now, false ))
			loaded++;
	}
...
extern "C" void LogError(AMX *amx, int err, const char *fmt, ...)
{
	...
	if (!pDebugger)
	{
		...
		if (err != AMX_ERR_EXIT)
		{
			AMXXLOG_Error("[AMXX] To enable debug mode, move plugin file to debug folder inside plugins folder");
		}
