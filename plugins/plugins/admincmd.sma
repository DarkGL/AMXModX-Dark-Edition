...
public cmdWho(id, level, cid)
{
	...
	for (new a = 0; a < inum; ++a)
	{
		...
		console_print(id, "%2d  %-16.15s %-20s %-8d %-6.5s %-6.5s %s %s%s", players[a], name, authid, 
		get_user_userid(players[a]), (flags&ADMIN_IMMUNITY) ? lYes : lNo, (flags&ADMIN_RESERVATION) ? lYes : lNo, sflags , userinfo[players[a]][UserIP], (userinfo[players[a]][UserDynamicIP]) ? ", dynamiczny adres IP" : "" );
	}
